...
> Declarative Nix packaging and home-manager module for OpenClaw. Reproducible deployment on macOS and Linux.

Source: `github:openclaw/nix-openclaw`
...
1. **[Determinate Nix](https://docs.determinate.systems/determinate-nix/)** installed on your machine
...
1. You message your Telegram bot for the first time
1. OpenClaw runs its **bootstrap ritual** - it asks you playful questions: *"Who am I? What am I? Who are you?"* - to learn its identity and yours
1. Once you've named it and introduced yourself, the bootstrap is done. You're up and running.
...
1. **Skills** - markdown files that teach the AI *how* to use those tools
...
1. For each plugin, Nix looks for a `openclawPlugin` output that declares:
   - What CLI packages to install
   - What skill files to copy
   - What environment variables it needs
1. Tools go on your PATH, skills get symlinked to `~/.openclaw/workspace/skills/`
1. A launchd (macOS) or systemd user service (Linux) is created/updated to run the gateway
1. The gateway starts, loads skills, connects to your providers
...
1. **Your Telegram user ID** - get from [@userinfobot](https://t.me/userinfobot), set `channels.telegram.allowFrom`
1. **Gateway auth token** - set `gateway.auth.token` (or `OPENCLAW_GATEWAY_TOKEN`) for the local gateway
1. **Provider API keys** - set via environment (e.g., `ANTHROPIC_API_KEY`) or `config.env.vars` (avoid secrets in store)
...
1. **Garnix** builds the package on Linux + macOS and runs `pnpm test` on Linux.\
   It also validates the generated Nix config options against the upstream schema.
1. **moltinators smoke test** runs against real Discord in `#moltinators-test`.
1. If green → promote to stable.
1. If red → keep current stable pin.
