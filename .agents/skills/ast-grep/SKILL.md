...
1. Add `stopBy: end` to relational rules if not present
1. Use `--debug-query` to understand the AST structure (see below)
1. Check if `kind` values are correct for the language
...
1. If that doesn't work, try `kind` to match the node type
1. Add relational rules (`has`, `inside`) as needed
1. Combine with composite rules (`all`, `any`, `not`) for complex logic
...
1. Check if metavariables are being detected correctly
1. Verify the node `kind` matches what you expect
1. Ensure relational rules are searching in the right direction
