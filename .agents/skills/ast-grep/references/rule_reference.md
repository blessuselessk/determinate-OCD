...
1. **Relational rule issues**: Ensure `stopBy: end` is set for deep searches
1. **Wrong node kind**: Check the language's Tree-sitter grammar for correct kind names
1. **Metavariable not working**: Ensure it's the only content in its AST node
1. **Pattern too complex**: Break it down into simpler sub-rules using `all`
